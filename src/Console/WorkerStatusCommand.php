...
final readonly class WorkerStatusCommand implements CommandInterface
{
    ...
    public function run(
        ...
        try {
            ...
            $output->error('CORETSIA_WORKER_STATUS_FAILED', 'worker-status-failed');
        }
        return 1;
    }
    ...
    private function validInput(
        ...
        if ($input->arguments() !== []) {
            $output->error('CORETSIA_WORKER_COMMAND_INVALID', 'worker-status-arguments-not-supported');
            return false;
        }
        if ($input->options() !== []) {
            $output->error('CORETSIA_WORKER_COMMAND_INVALID', 'worker-status-options-not-supported');
            return false;
        }
        return true;
    }
