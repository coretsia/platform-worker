...
- Forbidden:
  - any `core/*` package other than `core/contracts`, `core/foundation`, and `core/kernel`
...
`platform/worker` MUST NOT import or consume repository machinery under `tools/**`.
...
`WorkerServiceFactory::workerChildCommandBuilder(...)` derives one validated application-root-relative artifact root from:
...
The absolute artifact root MUST be a strict descendant of the application root.

An equal root or a path outside the application root fails deterministically.

`WorkerChildCommandBuilder` retains one application-root-relative artifact root and builds the exact child argv vector for both drivers.

`PcntlWorkerProcessDriver` retains the normalized application root, the package-owned launcher command, the command builder, readiness channel, and `WorkerProcessGuardianInterface`. It does not fork, signal, wait, or reap worker processes directly and does not receive `ContainerInterface` or `ApplicationWorker`.

`ProcWorkerProcessDriver` retains the normalized application root, one normalized child command vector, the command builder, readiness channel, and `WorkerProcessGuardianInterface`.
...
The child uses its working directory as the normalized application root and resolves the relative artifact root against it.
...
- configurable runtime paths must be application-root-relative.
- runtime paths must not be absolute.
- runtime paths must not contain `..`, backslashes, whitespace, control characters, `://`, or segments beginning with `@`.

`ConfigKernel` Phase B validation rules enforce the Worker-specific `@` segment constraint through `forbiddenSegmentPrefixes`. `WorkerPoolSpec` repeats the same path-safety checks as runtime defense in depth.
...
- [Worker package source](https://github.com/coretsia/monorepo/tree/main/packages/platform/worker)
